...
- La regresión con la función Ridge se conoce como ***regresión de Lasso*** y la regresión con la función Lasso se conoce como ***regresión Ridge***. La primera se utiliza cuando las variables independientes están correlacionadas y la segunda cuando las variables independientes están correlacionadas y se desea hacer selección de variables.
...
<center>
![](img/ridge_lasso_ols.png){width="100%" height="550px"}
</center>
...
<center>
![](img/Normal_ols.jpg){width="100%" height="550px"}
</center>
...
<center>
![](img/Linear_regresi'_ouliers.jpg){width="100%" height="550px"}
</center>

## 

<center>
![](img/outliers.jpg){width="100%" height="550px"}
</center>
...
<center>
![](img/mem_stats_psycho.jpg){width="100%" height="550px"}
</center>

##

<center>
![](img/moments_joke.jpg){width="100%" height="550px"}
</center>

##

<center>
![](img/iceberg_linear_regression.jpg){width="100%" height="550px"}
</center>

##

<center>
![](img/linear_r_types.jpg){width="100%" height="550px"}
</center>

##

<center>
![](img/linear_regression_assumptions.jpg){width="100%" height="550px"}
</center>

##

<center>
![](img/linear_regression_calculation.jpg){width="100%" height="550px"}
</center>

##

<center>
![](img/lm_joke.jpg){width="100%" height="550px"}
</center>

## 

<center>
![](img/lm_mem.jpg){width="100%" height="550px"}
</center>

##

<center>
![](img/lr_mem.jpg){width="100%" height="550px"}
</center>

##

<center>
![](img/lr_easy.jpg){width="100%" height="550px"}
</center>
...
- Sin embargo, se puede utilizar la regresión lineal para probar la causalidad. Por ejemplo, si se tiene un experimento aleatorizado, entonces se puede utilizar la regresión lineal para probar la causalidad. 

    - Aquellos interesados en la causalidad pueden consultar el libro de ***Causal Inference: The Mixtape*** de Scott Cunningham, y si quieren profundizar en el tema, pueden consultar el libro de ***Causality*** de Judea Pearl. Un libro para el público general es ***The Book of Why*** de Judea Pearl.
...
<center>
![](img/r_tramp.jpg){width="100%" height="550px"}
</center>

##

<center>
![](img/corelation_mem.jpg){width="100%" height="550px"}
</center>

##

<center>
![](img/causation_correlation.jpg){width="100%" height="550px"}
</center>

##

<center>
![](img/correlation_joke.jpg){width="100%" height="550px"}
</center>
...
- El valor de $\alpha$ se conoce como ***nivel de confianza***. Los valores más comunes de $\alpha$ son 0.05, 0.01 y 0.1.

- Los intervalos de confianza se calculan a partir de la distribución de probabilidad de la estimación puntual.
...
- El intervalo de confianza para el efecto del largo del pétalo sobre el largo del sépalo es:

```{r echo = TRUE}
# Calculamos el intervalo de confianza para el efecto del largo del pétalo sobre el largo del sépalo
beta_1 - t_alpha*sqrt(MSE/sum((data$Petal.Length - mean(data$Petal.Length))^2))
beta_1 + t_alpha*sqrt(MSE/sum((data$Petal.Length - mean(data$Petal.Length))^2))
```
...
<center>
![](img/ci_mem.jpg){width="100%" height="550px"}
</center>
...
## Ejemplo 2: Máxima verosimilitud {.smaller}

::: {.nonincremental}
- Simulemos datos de una distribución Normal con media 10 y desviación estándar 2.
...
- Para estimar la media y la desviación estándar, primero debemos especificar una función de verosimilitud, en este caso la función de verosimilitud es:

$$
L(\mu, \sigma | x) = \prod_{i=1}^n \frac{1}{\sqrt{2 \pi \sigma^2}} 
e^{-\frac{(x_i - \mu)^2}{2 \sigma^2}}
$$

- Dado que es más fácil trabajar con el logaritmo de la función de verosimilitud, entonces la función de verosimilitud es:

$$
\begin{align*}
\ell(\mu, \sigma | x) &= \sum_{i=1}^n \log \left( \frac{1}{\sqrt{2 \pi \sigma^2}} e^{-\frac{(x_i - \mu)^2}{2 \sigma^2}} \right) \\
&= \sum_{i=1}^n \left( \log \left( \frac{1}{\sqrt{2 \pi \sigma^2}} \right) + \log \left( e^{-\frac{(x_i - \mu)^2}{2 \sigma^2}} \right) \right) \\
&= \sum_{i=1}^n \left( \log \left( \frac{1}{\sqrt{2 \pi \sigma^2}} \right) - \frac{(x_i - \mu)^2}{2 \sigma^2} \right) \\
&= \sum_{i=1}^n \left( -\frac{1}{2} \log \left( 2 \pi \sigma^2 \right) - \frac{(x_i - \mu)^2}{2 \sigma^2} \right) \\
\end{align*}
$$

- Utilzando propiedades de la sumatoria, podemos escribir la función de verosimilitud como:

$$
\ell(\mu, \sigma | x) = -\frac{n}{2} \log \left( 2 \pi \sigma^2 \right) - \frac{1}{2 \sigma^2} \sum_{i=1}^n (x_i - \mu)^2
$$


- Para estimar cada parámetro, se deriva la función de verosimilitud con respecto al parámetro y se iguala a cero.

$$
\begin{align*}
&\frac{\partial \ell(\mu, \sigma | x)}{\partial \mu} = \\
&-\frac{n}{2} \log \left( 2 \pi \sigma^2 \right) - \frac{1}{2 \sigma^2} \sum_{i=1}^n (x_i - \mu)^2 = \\
&-\frac{n}{2} \log \left( 2 \pi \sigma^2 \right) - \frac{1}{2 \sigma^2} \sum_{i=1}^n (x_i^2 - 2 x_i \mu + \mu^2) = \\
&-\frac{n}{2} \log \left( 2 \pi \sigma^2 \right) - \frac{1}{2 \sigma^2} \left( \sum_{i=1}^n x_i^2 -2\sum_{i=1}^n x_i \mu + \sum_{i=1}^n \mu^2 \right) = \\
&-\frac{n}{2} \log \left( 2 \pi \sigma^2 \right) - \frac{1}{2 \sigma^2} \left( \sum_{i=1}^n x_i^2 - 2 n \mu \bar{x} + n \mu^2 \right) 
\end{align*}
$$

- Ahora todo lo que no tiene $\mu$ se puede eliminar, ya que al derivar con respecto a $\mu$ se anula. Y se obtiene:
$$
\begin{align*}
&\frac{\partial \ell(\mu, \sigma | x)}{\partial \mu} = \\
&-\frac{n}{2} \log \left( 2 \pi \sigma^2 \right) - \frac{1}{2 \sigma^2} \left( \sum_{i=1}^n x_i^2 - 2 n \mu \bar{x} + n \mu^2 \right) = \\
&-\frac{1}{2 \sigma^2} \left(- 2 n \mu \bar{x} + n \mu^2 \right) = \\
&\frac{n \mu^2}{2 \sigma^2} - \frac{n \mu \bar{x}}{\sigma^2} = \\
&\frac{n \mu}{\sigma^2} - \frac{n \bar{x}}{\sigma^2} = 0
\end{align*}
$$
...
$$
\begin{align*}
&\frac{n \mu}{\sigma^2} - \frac{n \bar{x}}{\sigma^2} = 0 \Rightarrow \\
&\frac{n \mu}{\sigma^2} = \frac{n \bar{x}}{\sigma^2} \Rightarrow \\
&n \mu = n \bar{x} \Rightarrow \\
&\mu = \bar{x}
\end{align*}
$$

- Con un proceso similar se puede calcular el valor puntual de la desviación estándar. El valor puntual de la media es $\bar{x}$ y el valor puntual de la desviación estándar es $s$, que es la desviación estándar muestral.
:::
...
::: {.nonincremental}
- Dado que son dos parámetros, se puede graficar la función de verosimilitud en 3D:
```{r echo = TRUE}
library(plotly)
mu <- seq(8, 12, 0.01)
sigma <- seq(1, 2.5, 0.01)
L <- matrix(0, nrow = length(mu), ncol = length(sigma))

for (i in 1:length(mu)) {
    for (j in 1:length(sigma)) {
        L[i, j] <- sum(dnorm(data, mean = mu[i], sd = sigma[j], log = T))
    }
}

plot_ly(x = mu, y = sigma, z = L, type = "surface") %>% 
    layout(
        title = "",
        scene = list(
            xaxis = list(title = "\u03BC"),
...
- Y podemos verificar nuestras estimaciones.

```{r echo = FALSE}
max_log = which(L == max(L), arr.ind = TRUE)   
print(paste("Max Log-Likelihood: ", L[max_log]))
print(paste("Media muestral = ", mean(data)))
print(paste("Desviación Estandar muestral = ", sd(data)))
print(paste("Estimación Puntual de la media = ", mu[187]))
print(paste("Estimación Puntual de la desv. est. = ", sigma[94]))
```
...
- Los ***modelos lineales generalizados*** son una generalización de la regresión lineal que permite modelar variables dependientes que no siguen una distribución Normal.

- Los modelos lineales generalizados se pueden ver como una distribución de probabilidad con una función de enlace.

- La ***función de enlace (link function)*** es una función que relaciona la media de la distribución con la variable independiente.

- Los modelos lineales generalizados se pueden ver como:
$$
g(E(y)) = \beta_0 + \beta_1 x_1 + \ldots + \beta_p x_p + \epsilon
$$

- Donde $g(E(y))$ es la función de enlace, $E(y)$ es el valor esperado de la variable dependiente, $\beta_0$ es el intercepto, $\beta_p$ es el efecto de la variable independiente $p$ sobre la variable dependiente y $x_p$ es el valor de la variable independiente $p$.

## Funciones de enlace {.smaller}

* Algunas funciones de enlace son:
$$
\begin{array}{|c|c|c|}
\hline
\text{Distribución} & \text{Función de enlace} & \text{Rango} \\
\hline
\text{Normal} & \text{Identidad} = E(y) & (-\infty, \infty) \\
\hline
\text{Bernoulli} & \text{Logit} = \log \left( \frac{p}{1 - p} \right) & (0, 1) \\
\hline
\text{Poisson} & \text{Log} = \log(\lambda) & (0, \infty) \\
\hline
\end{array}
$$

- Cada función de enlace tiene sus propiedades y se utiliza en problemas específicos.

## Supuestos de los modelos GLM {.smaller}

- Los datos son independientes.

- La variable dependiente sigue una distribución de probabilidad de la familia exponencial (Normal, Binomial, Poisson, Gamma, etc.).

- Los modelos GLM no asumen una relación lineal entre la variable dependiente y las variables independientes, pero si asumen una relación lineal entre la variable dependiente y la función de enlace. Por ejemplo, en el caso de la regresión logística, la relación entre la variable dependiente y la variable independiente es no lineal, pero la relación entre la variable dependiente y la función de enlace es lineal.
$$
\log \left( \frac{p}{1 - p} \right) = \beta_0 + \beta_1 x_1 + \ldots + \beta_p x_p
$$

- No se necesita que las variables sean homocedásticas.

- Los errores tienen que ser independientes pero no neceistan seguir una distribución Normal.

## Regresión logística {.smaller}

- La ***regresión logística*** es un modelo lineal generalizado que se utiliza para modelar variables dependientes binarias, como por ejemplo, si un paciente tiene o no una enfermedad.

- La regresión logística se puede ver como:
$$
\log \left( \frac{p}{1 - p} \right) = \beta_0 + \beta_1 x_1 + \ldots + \beta_p x_p
$$

- La cual utiliza la distribución logística como función de enlace para modelar variables bernoulli.
$$
p = \frac{e^{\beta_0 + \beta_1 x_1 + \ldots + \beta_p x_p}}{1 + e^{\beta_0 + \beta_1 x_1 + \ldots + \beta_p x_p}}
$$

- Observemos la forma de la distribución logística:

![](img/logistic_regression_plot.png){width="100%" height="550px"}

##

![](img/logistic_regresion.jpg){width="100%" height="550px"}

## Características de la regresión logística {.smaller}

- Algunas características de la curva de la regresión logística son:

    - La curva de la regresión logística es sigmoide, es decir, tiene forma de S.

    - La curva de la regresión logística tiene un rango de 0 a 1.   

    - La curva de la regresión logística es simétrica alrededor de 0.5. Y ese es el valor de la probabilidad cuando la combinación lineal de los parámetros es 0.

## Ejemplo 3: Regresión logística {.smaller}

:::{.nonincremental}

- Utilizemos una base de datos para analizar los datos de pacientes con enfermedad coronaria. Los datos son:
```{r echo = FALSE}
# Datos
...
- Visualicemos los pacientes con la enfermedad coronaria:
```{r echo = TRUE}

disease <- data$TenYearCHD
ggplot(data.frame(disease), aes(x = disease)) +
    geom_bar(fill = "#F8766D", color = "white", alpha = 0.8) +
    scale_x_continuous(breaks = c(0, 1), labels = c("No", "Sí")) +
    labs(x = "Enfermedad Coronaria", y = "Frecuencia") +
    dark_theme_gray() +
    theme(axis.text = element_text(size = 30),
        axis.title = element_text(size = 40),
        plot.title = element_text(size = 50))
...
- Podemos ver que la mayoría de los pacientes no tienen enfermedad coronaria. Esto es un problema de datos desbalanceados. Dado que la mayoría de los pacientes no tienen enfermedad coronaria, el modelo de regresión logística va a tener un sesgo hacia la clase mayoritaria.

- Veamos el resumen de los datos:
```{r echo = TRUE}
# Resumen de los datos
knitr::kable(summary(data), caption = "Resumen de los datos")
```

- Podemos incluso visualizar si existe correlación entre las variables:
```{r echo = TRUE}
# Correlación entre las variables
library(corrplot)

corrplot(cor(data[, c(2:4, 6:16)]), method = "color", 
        tl.col = "black", tl.srt = 45, bg = "white")
```

- Estimemos un modelo de regresión logística para predecir la enfermedad coronaria:
```{r echo = TRUE}
...
# Estimamos el modelo
logistic_model <- glm(TenYearCHD ~ ., data = data, family = binomial(link = "logit"))
...
## Regresión Logística en Psicología {.smaller}

- Existen modelos de regresión logtica para variables dependientes multinomiales, es decir, variables dependientes con más de dos categorías.

- También existen modelos de regresión logística para variables dependientes ordinales, es decir, variables dependientes con categorías ordenadas. En este caso se utiliza la función de enlace logit acumulada. Y se nombran modelos de regresión logística ordinal.

- Todos los modelos de regresión logística mencionados se suelen utilizar en psicometría para modelar variables latentes de habilidades y actitudes de los individuos, como por ejemplo, la inteligencia, la ansiedad, la depresión, etc.

- Cuando el test tiene ítems dicotómicos, es decir, ítems con dos categorías, se utiliza la regresión logística para modelar la probabilidad de que un individuo responda correctamente un ítem. Con más de dos categorías se utiliza la regresión logística multinomial. Con categorías ordenadas se utiliza la regresión logística ordinal.

## Modelo de Rasch {.smaller}

- El ***modelo de Rasch*** es un modelo lineal generalizado que se utiliza para medir habilidades y actitudes. Pertenece a la familia de los modelos de respuesta a ítem.

- El modelo de Rasch se puede ver como:
$$
\log \left( \frac{p}{1 - p} \right) = \theta - \beta_i
$$

- Donde $p$ es la probabilidad de que un individuo $i$ responda correctamente un ítem, $\theta$ es la habilidad del individuo $i$ y $\beta_i$ es la dificultad del ítem $i$.

- La probabilidad de que un individuo $i$ responda correctamente un ítem $j$ es:
$$
p_{ij} = \frac{e^{\theta_i - \beta_j}}{1 + e^{\theta_i - \beta_j}}
$$

- El modelo de Rasch es un modelo de regresión logística donde la variable dependiente es la respuesta correcta a un ítem y la variable independiente es la diferencia entre la habilidad del individuo y la dificultad del ítem.

## Estimación de parámetros {.smaller}

- La estimación de parámetros en los modelos lineales generalizados se realiza utilizando la máxima verosimilitud o con métodos bayesianos. Es por ello que es importante conocer la máxima verosimilitud.

- Estimar los parámetros en el modelo de Rasch es un proceso complicado y normalmente se realiza en dos pasos. En el primer paso se estima la dificultad de los ítems y en el segundo paso se estima la habilidad de los individuos. Y se requiere resolver un conjunto de ecuaciones.

- Con el fin de no complicar el curso, no veremos la estimación de parámetros en el modelo de Rasch completo.

## Ejemplo 3: Modelo de Rasch {.smaller}

:::{.nonincremental}

- Simulemos datos para el modelo de Rasch para 100 individuos y 10 ítems.
...
for(i in 1:n){
    for(j in 1:m){
        p[i, j] <- exp(theta[i] - beta[j])/(1 + exp(theta[i] - beta[j]))
    }
}
...
for(i in 1:n){
    for(j in 1:m){
        responses[i, j] <- rbinom(1, size = 1, prob = p[i, j])
    }
}
...
ggplot(data, aes(x = Var1, y =Var2, fill = value)) +
    labs(x = "Ítem", y = "Persona") +
    geom_raster(hjust = 0, vjust = 0) +
    scale_fill_gradient("Respuesta", low = "#6C1D7D", high = "#316995") +
...
- En este caso, la variable dependiente es la respuesta correcta a un ítem y la variable independiente es la diferencia entre la habilidad del individuo y la dificultad del ítem.

- La función de verosimilitud es muy complicada, para estimarse de forma analítica se requiere resolver un conjunto de ecuaciones. Por motivos de morbo matemático veamos la expresión general de la función de verosimilitud.
$$
L(\theta, \beta | x) = \prod_{i=1}^n \prod_{j=1}^m \left( \frac{e^{\theta_i - \beta_j}}{1 + e^{\theta_i - \beta_j}} \right)^{x_{ij}} \left( \frac{1}{1 + e^{\theta_i - \beta_j}} \right)^{1 - x_{ij}}
$$

- No se puede resolver de forma analítica, por lo que se utiliza un método numérico para estimar los parámetros. En R tenemos la función `RM` del paquete `eRm` que estima los parámetros del modelo de Rasch.
...
betas <- coef(rasch_model)
thetas <- person.parameter(rasch_model)

betas <- -coef(rasch_model)
thetas <- coef(person.parameter(rasch_model))

mes <- paste("Habilidad: ", thetas, sep = "")
mbs <- paste("Dificultad: ", betas, sep = "")

print(mes)
print(mbs)
```

- Por último podemos comparar las habilidades estimadas con las habilidades reales.

```{r echo = FALSE}
# Graficamos las habilidades estimadas
data_thetas <- data.frame(theta, thetas)
ggplot(data_thetas, aes(x = theta, y = thetas)) +
    geom_point(color = "#F8766D", size = 6) +
    geom_abline(intercept = 0, slope = 1, color = "white", size = 1.5) +
    labs(y = "Habilidad Estimada", x = "Habilidad Real") +
    dark_theme_gray() +
    theme(axis.text = element_text(size = 20),
        axis.title = element_text(size = 30),
        plot.title = element_text(size = 50))
...
- E igualmente podemos comparar las dificultades estimadas con las dificultades reales.

```{r echo = FALSE}
# Graficamos las dificultades estimadas
data_betas <- data.frame(beta, betas)

ggplot(data_betas, aes(x = beta, y = betas)) +
    geom_point(color = "#F8766D", size = 6) +
    geom_abline(intercept = 0, slope = 1, color = "white", size = 1.5) +
    labs(y = "Dificultad Estimada", x = "Dificultad Real") +
    dark_theme_gray() +
    theme(axis.text = element_text(size = 20),
        axis.title = element_text(size = 30),
        plot.title = element_text(size = 50))
...
## Propiedades del estimador de máxima verosimilitud {.smaller}

- Como vimos anteriormente, un buen estimador debe ser insesgado, consistente y eficiente.

- El estimador de máxima verosimilitud tiene ciertas propiedades que lo hacen un buen estimador.

    1. **Consistencia:** El estimador de máxima verosimilitud es consistente, es decir, cuando el tamaño de la muestra tiende a infinito, el estimador de máxima verosimilitud converge al valor real del parámetro.

    2. **Invarianza:** El estimador de máxima verosimilitud es invariante a la transformación de la función de verosimilitud. Es decir, si se transforma la función de verosimilitud, entonces el estimador de máxima verosimilitud no cambia.

    3. **Eficiencia:** El estimador de máxima verosimilitud es el estimador más eficiente. Es decir, el estimador de máxima verosimilitud tiene la menor varianza entre todos los estimadores insesgados.

    4. **Asintóticamente normal:** El estimador de máxima verosimilitud es asintóticamente normal. Es decir, cuando el tamaño de la muestra tiende a infinito, el estimador de máxima verosimilitud sigue una distribución Normal, con media 0 y varianza igual a la inversa de la información de Fisher.

##

<center>
![](img/statistics.jpg){width="100%" height="550px"}
</center>

##

<center>
![](img/statistics_water.jpg){width="100%" height="550px"}
</center>